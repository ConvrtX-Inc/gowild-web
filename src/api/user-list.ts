...
export const getAllUsers = async (): Promise<AxiosResponse> => {
  return await axios.get(`${USERS_URL}?sort=created_date,ASC`, CONFIG);
};
...
export const  getUsersByPage = async (
  ...
  return await axios.get(
    `${USERS_URL}?sort=updated_date,ASC&limit=${limit}&page=${page}`,
    CONFIG
  );
};

export const getStatusById = async (id: string): Promise<AxiosResponse> => {
  return await axios.get(`${STATUS_URL}/${id}`, CONFIG);
};
