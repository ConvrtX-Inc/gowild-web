...
import { Route } from 'src/lib/api/go-wild.api';
...
const RouteListTable: FC<RouteListTableProps> = (props) => {
  ...
  const { normalRoutes, isLoading, ...other } = props;
        ...
        <Scrollbar>
          <Box>
            <Table>
              ...
              <TableBody>
                {normalRoutes.length === 0 && isLoading === false && (
                  <TableRow>
                    <TableCell />
                    <TableCell />
                  </TableRow>
...
const StyledTablePagination = styled(TablePagination)<{ component?: 'div' }>`
  && {
    height: 43px;
    background: #ff7851;
