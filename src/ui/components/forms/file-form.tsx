...
export function FileForm({ multiple, name, ...props }: FileFormProps) {
  ...
  const handleDrop = useCallback(
    async (newFiles: File[], fileRejections: FileRejection[], event: DropEvent) => {
          ...
          <Box sx={{ position: 'relative' }}>
            {first.size > 1000000 ? (
              <Stack justifyContent='center' alignItems='center'>
                <GilroyMediumTypography color='#f44336'>
                  File must be 1MB size or less
                </GilroyMediumTypography>
                <Button
              ...
              <>
                ...
                <img height='90px' width='90px' src={!isEmpty ? first.path : ''} alt='route-img' />
              </>
            )}
          </Box>
        ) : (
