...
const apiIsLoaded = (map, maps, loadRouteMarkers, loadEventMarkers) => {
  ...
  startingPtMarker.addListener('click', () => {
    ...
    logger.debug(`Starting Pt Lat: ${lat}, Long: ${long}`);
  });
  ...
  finishingPtMarker.addListener('click', () => {
    ...
    logger.debug(`Finishing Pt Lat: ${lat}, Long: ${long}`);
  });

  // Automatically zoom and fit google map viewport based on available markers
  maps.event.addListenerOnce(map, 'tilesloaded', () => {
    logger.debug('Google Map Tiles loaded [Automatically zoom and fit based on all markers]');
  ...
  logger.debug('Placing Event Markers....  ');
  ...
  for (let i = 0; i < loadEventMarkers.length; i++) {
    ...
    logger.debug('Total Num of Event Markers: ', loadEventMarkers.length);
    if (i === loadEventMarkers.length - 1) {
      logger.debug('Placing Event Markers Complete  ');
    }

    maps.event.addListener(markers[i], 'click', () => {
      logger.debug(`Event Marker ${loadEventMarkers[i].closure_uid} clicked`);
    });
  }
...
const RouteViewMap: FC<any> = (props) => {
  ...
  logger.debug('Load route markers props: ', loadRouteMarkers);
  logger.debug('Load event markers props: ', loadEventMarkers);
